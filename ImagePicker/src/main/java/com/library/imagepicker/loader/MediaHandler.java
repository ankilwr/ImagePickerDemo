...
public class MediaHandler {
    ...
    public static List<MediaFolder> getMediaFolder(Context context, ArrayList<MediaFile> imageFileList, ArrayList<MediaFile> videoFileList) {
        ...
        for(MediaFile media: mediaFileList){
            Log.e("测试测试", String.format("uri:%s(%s): %s", index, media.getMime(), media.getUri()));
            Log.i("测试测试", String.format("path:%s(%s): %s", index, media.getMime(), media.getPath()));
            index++;
        }

        //全部图片或视频
        if (!mediaFileList.isEmpty()) {
            MediaFolder allMediaFolder = new MediaFolder(ALL_MEDIA_FOLDER, context.getString(R.string.all_media), mediaFileList);
            mediaFolderMap.put(ALL_MEDIA_FOLDER, allMediaFolder);
        }

        //全部视频
        if (videoFileList != null && !videoFileList.isEmpty()) {
            MediaFolder allVideoFolder = new MediaFolder(ALL_VIDEO_FOLDER, context.getString(R.string.all_video), videoFileList);
        ...
        if (imageFileList != null && !imageFileList.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (mediaFolder == null) {
                    mediaFolder = new MediaFolder(imageFolderId, mediaFile.getFolderName(), new ArrayList<MediaFile>());
                }
        ...
        for (Integer folderId : mediaFolderMap.keySet()) {
            mediaFolderList.add(mediaFolderMap.get(folderId).init());
        }
